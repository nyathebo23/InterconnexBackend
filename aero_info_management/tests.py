from .constants import CONTROLE_ACTION, CREATE_ACTION, DRAFT_STATE, PENDING_ADMISSION_STATE, PENDING_APPROVAL_STATE, PENDING_VALIDATION_STATE, PENDING_VERIFICATION_STATE
import datetime
import re
from rest_framework_simplejwt.tokens import RefreshToken
from .models import ActionAgentOnDDIA, Aerodrome, DDIAHistory, DemandeNOTAM, LocalAgent, LocalInformer, NationalInformer, Unit, Agent, Validation
...
class StructuresCRUTests(APICustomTestCase):
    ...
    def test_create_update_nationalinformer(self):
        ...
        resp = self.client.post(urlcreate, {"name": "ASECNA"}, **self.bearer_token)
        ...
        resp = self.client.put(urlupdate, {"name": "CCAA"}, **self.bearer_token)
    ...
    def test_create_update_localinformer(self):
        ...
        resp = self.client.post(urlcreate, {"name": "SEGC"}, **self.bearer_token)
        ...
        resp = self.client.put(urlupdate, {"name": "SEGC 2"}, **self.bearer_token)
    ...
    def test_create_update_aerodrome(self):
        ...
        resp = self.client.post(urlcreate, data, **self.bearer_token)
        ...
        resp = self.client.put(urlupdate, data2, **self.bearer_token)
    ...
    def test_create_update_unit(self):
        ...
        resp = self.client.patch(urlupdate, data2, **self.bearer_token)
...
class AgentActionsTest(APICustomTestCase):
    ...
    def test_create_ddia(self):
        ...
        data_notam = {
            'start_val_period': datetime.date(2021, 8, 22),
            'end_val_period': datetime.date(2021, 10, 22),
            'daily_freq_start': datetime.time(8, 30),
            'daily_freq_end': datetime.time(17, 30),
            'attachments': []
        }

        resp = self.client.post(urlnotam, data_notam, **self.bearer_token)
        ...
        resp = self.client.post(urlsuppaip, data_suppaip, **self.bearer_token)
        ...
        resp = self.client.post(urlaic, data_aic, **self.bearer_token)
        ...
        client = APIClient()
        client.force_authenticate(user_verif)

        urllistddia = reverse('listDDIAwaited-verifsource', kwargs={'type_ddia': 'all'})
        resp = client.get(urllistddia)
        print(resp.data)
...
class AgentsControlsActionsTest(APITestCase):
    ...
    def setUp(self) -> None:
        self.user_init = User.objects.create_user(username="Nyat", email="franckhebo@gmail.com", password="Franck23.", role="Agent source", sex="male", is_staff=True)
        self.user_init.is_active = True
        self.user_init.save()
        self.refresh_user_init = RefreshToken.for_user(self.user_init)
        self.user_verif = User.objects.create_user(username="Lomta", email="talompatrick@gmail.com", password="Talom23.", role="Verificateur source", sex="male", is_staff=True)
        self.user_verif.is_active = True
        self.user_verif.save()
        self.refresh_user_verif = RefreshToken.for_user(self.user_verif)
        self.user_commander = User.objects.create_user(username="Asa", email="joelasa@gmail.com", password="Talom23.", role="Structure source", sex="male", is_staff=True)
        ...
        self.user_localinf = User.objects.create_user(username="KONRAD", email="hebogerland@gmail.com", password="Talom23.", role="Informateur local", sex="male", is_staff=True)
        ...
        unit = Unit.objects.create(email="doualaaero@gmail.com", name="AIM", phone_number="+26556566", fax="256656", address="azertgyh", aerodrome=aerodrome) 
        agent_init = Agent.objects.create(user=self.user_init, unit=unit0, source_structure=aerodrome)   
        agent_verif = Agent.objects.create(user=self.user_verif, unit=unit, source_structure=aerodrome)
        agent_command = Agent.objects.create(user=self.user_commander, unit=unit, source_structure=aerodrome)
        local_agent = LocalAgent.objects.create(user=self.user_localinf, localinformer=localinf)
        # return {"HTTP_AUTHORIZATION":f'Bearer {refresh.access_token}'}
    ...
    def test_controls_on_ddia(self):
        ...
        resp = self.client.post(urlcreate, data_notam, HTTP_AUTHORIZATION=f'Bearer {self.refresh_user_init.access_token}')   
        ...
        resp = self.client.post(urlverif, data_accept, HTTP_AUTHORIZATION=f'Bearer {self.refresh_user_verif.access_token}') 
        ...
        resp = self.client.get(urllistddia, HTTP_AUTHORIZATION=f'Bearer {self.refresh_user_verif.access_token}')
        print(resp.data)

...
class IsVerifier(permissions.BasePermission):
    def has_permission(self, request, view):
        user = request.user
        user_role = user.role
        islocalinf = request.GET.get('is_localinf')
        isLocalInf = LocalAgent.objects.filter(user=user).exists()
        if islocalinf is not None:
            if islocalinf == 'yes':
                return isLocalInf and user_role == SOURCE_VERIFIER

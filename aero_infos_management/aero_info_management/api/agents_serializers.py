...
class NationalAgentSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        user = data.get('user')
        if user.role != NATIONAL_INFORMER:
...
class LocalAgentSerializerForHistory(serializers.ModelSerializer):
    ...
    localinformer_name = serializers.SerializerMethodField(read_only=True)
    aerodrome_name = serializers.SerializerMethodField(read_only=True)
    class Meta:
        model = LocalAgent
        fields = '__all__'

    def get_localinformer_name(self, obj):
        return obj.localinformer.name
    
    def get_aerodrome_name(self, obj):
        if obj.localinformer.aerodrome is None:
            return ''
        return obj.localinformer.aerodrome.name
...
class NationalAgentSerializerForHistory(serializers.ModelSerializer):
    ...
    class Meta:
        model = NationalAgent
        fields = ['id', 'user', 'nationalinf_name']
...
class AgentSerializerForHistory(serializers.ModelSerializer):
    ...
    class Meta:
        model = Agent
        fields = ('id', 'user', 'unit_name', 'aerodrome_name') 

...
def remove_notif(notif: Notification):
...
def notify_sourcestructure_ddia_verification(actionObj, typeDDIA: str, aerodrome: Aerodrome, refDDIA: str, request):
    ...
    if isinstance(actionObj, LocalInformerAction):
        ...
        data = SourceStructureActionSerializer(actionObj, context={'request': request}).data
    notif = Notification.objects.filter(event='ddia-reception-submission', ref_ddia=refDDIA).first()
    remove_notif(notif)
...
def notify_ddia_reject(ddia: DDIA, typeDDIA: str, request):
    notification = Notification.objects.create(receiver_object=ddia.unit, event='ddia-reject', 
    new_ddia_state=ddia.state, ddia_type=typeDDIADict.get(typeDDIA), ref_ddia=ddia.ident_ddia)
    data = None
    if typeDDIA == 'demandenotam':
        data = DemandeNOTAMItemListSerializer(ddia, context={'request': request}).data
    elif typeDDIA == 'demandesupp':
        data = DemandeSUPPItemListSerializer(ddia, context={'request': request}).data
    elif typeDDIA == 'demandeaic':
        data = DemandeAICItemListSerializer(ddia, context={'request': request}).data    
    pusher_client.trigger('unit'+str(ddia.unit.id), 'ddia-reject', data={
        'data': data,
        'notification': NotificationSerializer(notification).data
    })
...
def get_notifications_sourceunit(request: HttpRequest):
    ...
    notifications = Notification.objects.filter(unit=unit, is_valid=True).exclude(event='ddia-state-change')
